...
scala -cp cafebabe_2.11-1.2.jar:koolc_2.11-1.3.1.jar koolc.Main $1 1> /dev/null
classfile=$(grep object $1 | awk '{ print $2 }')
java $classfile > reference.res
echo "Running ours..."
scala -cp target/scala-2.11/classes:cafebabe_2.11-1.2.jar koolc.Main $1 1> /dev/null
